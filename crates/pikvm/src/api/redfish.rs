...
impl RedfishApi<'_> {
    ...
    pub async fn root(&self) -> Result<Value> {
        let r = self.0.call_ok(Method::GET, "/api/redfish/v1", &Query::new(), Body::Empty).await?;
        r.json()
    }

    /// `GET /api/redfish/v1/Systems`
    pub async fn systems(&self) -> Result<Value> {
        let r = self.0.call_ok(Method::GET, "/api/redfish/v1/Systems", &Query::new(), Body::Empty).await?;
        r.json()
    }
    ...
    pub async fn patch_system(&self, id: &str) -> Result<()> {
        let path = format!("/api/redfish/v1/Systems/{id}");
        self.0.call_ok(Method::PATCH, &path, &Query::new(), Body::Json(json!({}))).await?;
        Ok(())
    }
